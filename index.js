...
function filterUnfundedOnly() {
  ...
  let listOfUnmetFunding = GAMES_JSON.filter((games) => {
    return games.pledged < games.goal;
  });

  // use the function we previously created to add the unfunded games to the DOM
  addGamesToPage(listOfUnmetFunding);
}
...
function filterFundedOnly() {
  ...
  let listOfMetFunding = GAMES_JSON.filter((games) => {
    return games.pledged >= games.goal;
  });

  // use the function we previously created to add unfunded games to the DOM
  addGamesToPage(listOfMetFunding);
}
...
let [g1, g2, ...restofgames] = sortedGames;

// create a new element to hold the name of the top pledge game, then append it to the correct element
const firstGame = document.createElement("firstGame");
firstGame.innerHTML = `<p>${g1.name}</p>`;
firstGameContainer.appendChild(firstGame);

// do the same for the runner up item
const secondGame = document.createElement("secondGame");
secondGame.innerHTML = `<p>${g2.name}</p>`;
